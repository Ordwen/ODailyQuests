...
public enum SQLQuery {
    ...
    H2_PLAYER_QUERY("""
        MERGE INTO `odq_player` (`player_uuid`, `player_timestamp`, `achieved_quests`, `total_achieved_quests`)
        KEY (`player_uuid`) VALUES (?, ?, ?, ?);
    """),

    H2_PROGRESS_UPDATE("""
        MERGE INTO `odq_progression` (`player_uuid`, `player_quest_id`, `quest_index`, `advancement`, `is_achieved`)
        KEY (`player_uuid`, `player_quest_id`) VALUES (?, ?, ?, ?, ?);
    """),
